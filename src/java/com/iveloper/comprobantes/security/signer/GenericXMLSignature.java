...
public abstract class GenericXMLSignature {
     ...
     */
    protected ByteArrayOutputStream execute(Certificate cert) throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException, CertStoreException, Exception {
     ...
     */
    private IPKStoreManager getPKStoreManager(Certificate cert) throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException {

        IPKStoreManager storeManager = null;
        try {
            KeyStore ks = KeyStore.getInstance("PKCS12");
            ks.load(cert.getContent(), cert.getPass().toCharArray());
            storeManager = new KSStore(ks, new PassStoreKS(cert.getPass()));
